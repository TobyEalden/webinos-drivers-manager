    ...
    function intReg() {
        ...
        if (existsSync(CONFIG_PATH)) {
          ...
          for (var dev in configData.devices) {
            ...
            var sensor_info = { type:"temperature", name: configData.devices[dev].name + " temperature", description:configData.devices[dev].name + " [" + dev + "]"};
            device.temperatureSensorServiceId = registerFunc(driverId, 0, sensor_info);
            var actuator_info = { type:"temperature", name: configData.devices[dev].name + " valve", description:configData.devices[dev].name + " [" + dev + "]"};
            device.valveSensorServiceId = registerFunc(driverId, 0, actuator_info);
          }
